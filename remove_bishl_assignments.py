...
parser = argparse.ArgumentParser(description="Remove all team assignments and set managedByISHD=True for eligible players.")
parser.add_argument("--prod", action="store_true", help="Update production database.")
parser.add_argument("--dry-run", action="store_true", help="Show what would be modified without actually doing it.")
...
def remove_bishl_assignments():
    ...
    query = {
        "managedByISHD": {"$ne": True},
        "assignedTeams.teams.teamType": {"$ne": "HOBBY"}
    }
    ...
    for player in players_list:
        ...
        if args.dry_run:
            ...
            if result.modified_count > 0:
                ...
                print(f"  - ✗ No changes made (already processed?)")
