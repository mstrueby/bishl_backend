    ...
    if first_name:
        ...
        if current_sex != predicted_sex:
            update_result = db["players"].update_one(
                {"_id": player["_id"]}, {"$set": {"sex": predicted_sex}}
            )

            if update_result.modified_count > 0:
                updated_players += 1
                print(
                    f"  ✅ Updated player: {player.get('firstName', 'Unknown')} {player.get('lastName', 'Unknown')} - {current_sex} -> {predicted_sex}"
                )
            else:
                print(
                    f"  ❌ Change not successfull for player: {player.get('firstName', 'Unknown')} {player.get('lastName', 'Unknown')} - {current_sex}"
                )
        # else:
