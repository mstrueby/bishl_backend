...
async def repair_assignment_conflicts(is_prod=False):
    ...
    print(
        f"Repairing assignment conflicts in {'PRODUCTION' if is_prod else 'DEVELOPMENT'} database..."
    )
            ...
            async with await client.start_session() as session:
                async with session.start_transaction():
                    try:
                        ...
                        print(
                            f"✓ Repaired: Assignment {assignment.get('_id')} - Match {match_id} - Position {position} - Referee {referee['firstName']} {referee['lastName']}"
                        )
                        repaired += 1
...
async def main():
    parser = argparse.ArgumentParser(
        description="Repair assignment-match synchronization conflicts"
    )
    parser.add_argument(
