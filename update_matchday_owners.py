...
tournaments = db["tournaments"].find({"seasons.alias": "2025"})
                ...
                for matchday_idx, matchday in enumerate(round_data.get("matchdays", [])):
                    ...
                    if matchday_alias in matchday_owners:
                        ...
                        result = db["tournaments"].update_one(
                            {"_id": tournament["_id"]}, {"$set": {update_path: owner_obj}}
                        )

                        if result.modified_count > 0:
                            updated_count += 1
                            print(
                                f"      ✅ Updated owner for matchday: {matchday.get('name', 'unknown')} ({matchday_alias}) to {owner['clubName']}"
                            )
                        else:
                            print(
                                f"      ⚠️ No changes made for matchday: {matchday.get('name', 'unknown')} ({matchday_alias})"
                            )
                    else:
                        print(
                            f"      ℹ️ No owner data for matchday: {matchday.get('name', 'unknown')} ({matchday_alias})"
                        )
