...
async def check_password_migration_status(is_prod: bool = False):
    ...
    try:
        ...
        stats = {"total": len(users), "argon2": 0, "bcrypt": 0, "other": 0}
        ...
        print(
            f"  ✅ Argon2 (migrated): {stats['argon2']} ({stats['argon2']/stats['total']*100:.1f}%)"
        )
        print(
            f"  ⏳ Bcrypt (needs migration): {stats['bcrypt']} ({stats['bcrypt']/stats['total']*100:.1f}%)"
        )
        if stats["other"] > 0:
        ...
        if users_by_type["bcrypt"]:
            ...
            for i, user in enumerate(users_by_type["bcrypt"], 1):
                ...
                print(
                    f"{i:3d}. {user['email']:40s} | {user['firstName']} {user['lastName']:20s} | {roles_str}"
                )
