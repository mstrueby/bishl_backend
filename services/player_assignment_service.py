...
class PlayerAssignmentService:
    ...
    WKO_RULES: list[WkoRule] = [
        ...
        WkoRule(
            ...
            secondaryRules=[
                ...
                SecondaryRule(
                    targetAgeGroup="DAMEN", sex=[Sex.FEMALE], maxLicenses=99, requiresAdmin=False
                ),
            ],
            maxTotalAgeClasses={Sex.MALE: 2, Sex.FEMALE: 2},
        ),
    ...
    async def classify_license_types_for_player(self, player: dict) -> dict:
        ...
        if len(all_licenses) == 1:
            ...
            # An explicitly-stored non-UNKNOWN type (LOAN, SPECIAL, SECONDARY, etc.)
            # was intentionally set by an admin — leave it unchanged.
            if team.get("licenseType") not in [LicenseType.UNKNOWN, None]:
                return player
    ...
    def _validate_age_group_compliance(
        self, player: dict, player_obj: PlayerDB, primary_club_ids: list[str] | None = None
    ) -> None:
        """
        ...
        for club in player["assignedTeams"]:
            for team in club.get("teams", []):
                ...
                is_invalid = (both_known and team_age_group != player_age_group) or (
                    not both_known
                    and not self._is_age_group_compatible(player_age_group, team_age_group)
                )
                if is_invalid:
                    team["status"] = LicenseStatus.INVALID
