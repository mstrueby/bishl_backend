...
async def get_database(use_production: bool = False):
    ...
    if not db_url:
        raise ValueError(
            f"Database URL not found. Set {'DB_URL_PROD' if use_production else 'DB_URL'} environment variable."
        )
...
async def migrate_match(db, match: dict, dry_run: bool = False) -> dict:
    ...
    if updates and not dry_run:
        await db["matches"].update_one({"_id": match_id}, {"$set": updates})
...
async def run_migration(dry_run: bool = False, use_production: bool = False):
    """Run the migration."""
    print(f"\n{'=' * 60}")
    print("Roster Structure Migration")
    print(f"{'=' * 60}")
...
def main():
    parser = argparse.ArgumentParser(description="Migrate match roster structure to nested format")
    parser.add_argument(
