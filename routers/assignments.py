    ...
    if send_emails:
        ...
        for club_id, club_matches in matches_by_club.items():
            try:
                ...
                <p>Bis zum {(target_date - timedelta(days=7)).strftime('%d.%m.')} können nur Schiedsrichter der beteiligten Vereine anfragen. Der Verein <strong>{club_name}</strong>) ist nun in der Verantwortung zwei Schiedsrichter zu stellen. Ab dem {(target_date - timedelta(days=6)).strftime('%d.%m.')} können wieder alle Schiedsrichter anfragen.</p>
                ...
                if admin_emails and os.environ.get('ENV') == 'production':
                    ...
                    if os.environ.get('ENV') == 'production':
                        ...
                        if admin_user_email:
                            ...
                            emails_sent += 1
                            print(f"Test email sent to admin user {admin_user_email} for {club_name} (would go to LIGENLEITUNG_EMAIL in production)")
                        else:
                            print(f"ADMIN_USER not set in environment, email not sent for {club_name}")
                else:
