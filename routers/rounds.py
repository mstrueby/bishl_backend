    ...
    if (
        ...
        for season in tournament.get("seasons", []):
            if season.get("alias") == season_alias:
                ...
                return JSONResponse(
                    status_code=status.HTTP_200_OK, content=jsonable_encoder(rounds)
                )
...
@router.get(
    "/{round_alias}", response_description="Get one round of a season", response_model=RoundDB
)
async def get_round(
    request: Request,
    tournament_alias: str = Path(
            ...
            if season.get("alias") == season_alias:
                for round in season.get("rounds", []):
                    if round.get("alias") == round_alias:
                        ...
                        return JSONResponse(
                            status_code=status.HTTP_200_OK, content=jsonable_encoder(round_response)
                        )
        raise ResourceNotFoundException(
            resource_type="Round",
...
async def add_round(
    ...
    if "ADMIN" not in token_payload.roles:
        raise AuthorizationException(
            message="Admin role required to add rounds", details={"user_roles": token_payload.roles}
        )

    logger.info(
        f"Adding round to {tournament_alias}/{season_alias}",
        extra={
    ...
    if (
        season := next((s for s in tournament["seasons"] if s["alias"] == season_alias), None)
    ) is None:
        raise ResourceNotFoundException(
            resource_type="Season",
            resource_id=season_alias,
    ...
    try:
        ...
        result = await mongodb["tournaments"].update_one(
            ...
            {"$push": {"seasons.$.rounds": round_data}},
        )
        # get inserted round
        if result.modified_count == 1:
            ...
            if updated_tournament and "seasons" in updated_tournament:
                ...
                if season_data and "rounds" in season_data:
                    new_round = next(
                        (r for r in season_data.get("rounds", []) if r["alias"] == round.alias),
                        None,
                    )
                    if new_round:
                    ...
                    raise HTTPException(
                        status_code=404, detail=f"Newly added round {round.alias} not found"
                    )
        ...
        logger.error(
            f"Failed to add round {round.alias}",
            extra={
                "error": str(e),
                "season_alias": season_alias,
                "tournament_alias": tournament_alias,
...
@router.patch(
    "/{round_id}", response_description="Update a round of a season", response_model=RoundDB
)
async def update_round(
    ...
    season_alias: str = Path(..., description="The alias of the season to update the round in"),
    round: RoundUpdate = Body(..., description="The data of the round to update"),
    token_payload: TokenPayload = Depends(auth.auth_wrapper),
):
    mongodb = request.app.state.mongodb
    if "ADMIN" not in token_payload.roles:
    ...
    round_index = next(
        (
            i
            for i, r in enumerate(tournament["seasons"][season_index].get("rounds", []))
            if r.get("_id") == round_id
        ),
        None,
    )
    if DEBUG_LEVEL > 20:
    ...
    for field in round_dict:
        if field != "_id" and round_dict[field] != tournament["seasons"][season_index]["rounds"][
            ...
            update_data["$set"][f"seasons.{season_index}.rounds.{round_index}.{field}"] = (
                round_dict[field]
            )
    if DEBUG_LEVEL > 10:
    ...
    if update_data.get("$set"):
        ...
        try:
            ...
            logger.error(
                f"Failed to update round {round_id}",
                extra={
                    "error": str(e),
                    "season_alias": season_alias,
                    "tournament_alias": tournament_alias,
    ...
    if updated_tournament and "seasons" in updated_tournament:
        season_data = next(
            (s for s in updated_tournament["seasons"] if s.get("alias") == season_alias), None
        )
        if season_data and "rounds" in season_data:
            updated_round = next(
                (r for r in season_data.get("rounds", []) if str(r.get("_id")) == round_id), None
            )
            if updated_round and "matchdays" in updated_round:
                if "matchdays" in updated_round and updated_round["matchdays"] is not None:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"Fetch: Round with id {round_id} not found in season {season_alias} of tournament {tournament_alias}",
            )
    else:
        raise HTTPException(
            status_code=404,
            detail=f"Fetch: Season {season_alias} not found in tournament {tournament_alias}",
        )
