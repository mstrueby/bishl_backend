...
async def send_message(
    ...
    receiver = await mongodb["users"].find_one({"_id": message_data["receiverId"]})
    if not receiver:
    ...
    if result.inserted_id:
        new_message = await mongodb["messages"].find_one({"_id": result.inserted_id})
        return JSONResponse(
            status_code=status.HTTP_201_CREATED, content=jsonable_encoder(MessageDB(**new_message))
        )
...
async def get_messages(
    ...
    messages = (
        await mongodb["messages"]
        .find({"receiver.userId": user_id})
        .sort("timestamp", -1)
        .to_list(length=100)
    )
    ...
    for message in messages:
        if not message.get("read", False):  # Default to False if 'read' field is missing
            await mongodb["messages"].update_one({"_id": message["_id"]}, {"$set": {"read": True}})
...
async def delete_message(
    ...
    delete_result = await mongodb["messages"].delete_one({"_id": message_id})
    ...
    raise HTTPException(
        status_code=404,
        detail="Message not found or you do not have permission to delete this message",
    )
...
async def get_chat_with_user(
    ...
    messages = (
        ...
        .find(
            {
                "$or": [
                    {"sender.userId": user_id, "receiver.userId": other_user_id},
                    {"sender.userId": other_user_id, "receiver.userId": user_id},
                ]
            }
        )
        .sort("timestamp", -1)
    ...
    for message in messages:
        if message["receiver"]["userId"] == user_id and not message.get(
            "read", False
        ):  # Default to False if 'read' field is missing
            await mongodb["messages"].update_one({"_id": message["_id"]}, {"$set": {"read": True}})
    return JSONResponse(status_code=status.HTTP_200_OK, content=jsonable_encoder(messages))
...
async def get_chatted_users(
    ...
    chatted_user_ids = list(set(sent_message_user_ids + received_message_user_ids))
    chatted_users = []
    for chat_user_id in chatted_user_ids:
        chat_user = await mongodb["users"].find_one(
            {"_id": chat_user_id}, {"firstName": 1, "lastName": 1}
        )
        if chat_user:
            unread_count = 0
            unread_count = await mongodb["messages"].count_documents(
                {"sender.userId": chat_user_id, "receiver.userId": user_id, "read": False}
            )
