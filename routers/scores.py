...
async def get_score_object(mongodb, match_id: str, team_flag: str, score_id: str) -> ScoresDB:
    """
    ...
    if "matchSeconds" in return_data:
        return_data["matchTime"] = parse_time_from_seconds(return_data["matchSeconds"])
...
async def create_score(
    ...
    if match_status != "INPROGRESS":
        raise HTTPException(
            status_code=400, detail="Scores can only be added when match status is INPROGRESS"
        )

    # check if score player is in roster
    if score.goalPlayer and score.goalPlayer.playerId:
        if not any(
            player["player"]["playerId"] == score.goalPlayer.playerId
            for player in match.get(team_flag, {}).get("roster") or []
        ):
            raise HTTPException(
                status_code=400, detail=f"Goal player {score.goalPlayer.playerId} not in roster"
            )

    if score.assistPlayer and score.assistPlayer.playerId:
        if not any(
            player["player"]["playerId"] == score.assistPlayer.playerId
            for player in match.get(team_flag, {}).get("roster") or []
        ):
            raise HTTPException(
                status_code=400, detail=f"Assist player {score.assistPlayer.playerId} not in roster"
            )
    ...
    try:
        ...
        if update_result.modified_count == 0:
            raise HTTPException(status_code=500, detail="Failed to update match with score")
        ...
        if DEBUG_LEVEL > 0:
            print(
                f"Score added with incremental updates - Goal: {goal_player_id}, Assist: {assist_player_id}"
            )
...
async def patch_one_score(
    ...
    if match_status != "INPROGRESS":
        raise HTTPException(
            status_code=400, detail="Scores can only be modified when match status is INPROGRESS"
        )
    ...
    updated_score = await get_score_object(mongodb, match_id, team_flag, score_id)
...
async def delete_one_score(
    ...
    if match_status != "INPROGRESS":
        raise HTTPException(
            status_code=400, detail="Scores can only be deleted when match status is INPROGRESS"
        )
    ...
    try:
        ...
        if finish_type and t_alias:
            ...
            standings_settings = await stats_service.get_standings_settings(
                match.get("tournament").get("alias"), match.get("season").get("alias")
            )
        ...
        if DEBUG_LEVEL > 0:
            print(
                f"Score deleted with incremental updates - Goal: {goal_player_id}, Assist: {assist_player_id}"
            )
