...
async def create_post(
    ...
    if not any(role in token_payload.roles for role in ["ADMIN", "AUTHOR"]):
        raise AuthorizationException(
            message="Admin or Author role required", details={"user_roles": token_payload.roles}
        )
    ...
    # revision
    revision = Revision(
        updateData=post_data, updateUser=post_data["createUser"], updateDate=post_data["createDate"]
    )
...
async def update_post(
    ...
    if alias:
        upd_alias = alias
        alias_suffix = 2
        while await request.app.state.mongodb["posts"].find_one(
            {"alias": upd_alias, "_id": {"$ne": id}}
        ):
    ...
    revision = Revision(
        updateData={
            k: v for k, v in post_to_update.items() if k != "updateUser" and k != "updateDate"
        },
    ...
    # Update post
    try:
        update_result = await mongodb["posts"].update_one({"_id": id}, {"$set": post_to_update})
