...
for user in users:
    ...
    if updates_needed:
        try:
            result = db["users"].update_one({"_id": user["_id"]}, {"$set": updates_needed})

            if result.modified_count > 0:
                updated_users += 1
                print(
                    f"✅ Updated logos for {user.get('firstName', '')} {user.get('lastName', '')}"
                )
            else:
                print(
                    f"⚠️ No changes made for {user.get('firstName', '')} {user.get('lastName', '')}"
                )
        except Exception as e:
            print(
                f"❌ Error updating {user.get('firstName', '')} {user.get('lastName', '')}: {str(e)}"
            )
    else:
