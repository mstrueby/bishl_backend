...
parser.add_argument("--importAll", action="store_true", help="Import all matches.")
parser.add_argument("--prod", action="store_true", help="Import matches to production.")
...
with open(filename, encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        if isinstance(tournament_data, dict) and all(
            isinstance(k, str) for k in tournament_data.keys()
        ):
            tournament = MatchTournament(**tournament_data)
        else:
        ...
        if isinstance(matchday_data, str):
            try:
                ...
                exit()
        if isinstance(matchday_data, dict) and all(
            isinstance(k, str) for k in matchday_data.keys()
        ):
        ...
        if any(obj is None for obj in [tournament, season, round, matchday]):
            print(
                "Error: One of the required objects (tournament, season, round, matchday) is None."
            )
            exit()
        ...
        if any(obj.alias is None for obj in [tournament, season, round, matchday]):
            print(
                "Error: One of the required fields (tournament, season, round, matchday) is None."
            )
            exit()
        else:
        ...
        if not round_data.matchdays or not any(md.alias == md_alias for md in round_data.matchdays):
            ...
            if isinstance(new_matchday_data, str):
                try:
                    ...
                    print(
                        f"Attempted to parse: {new_matchday_data}"
                    )  # Debug the invalid JSON content
                    exit()
            if isinstance(new_matchday_data, dict) and all(
                isinstance(k, str) for k in new_matchday_data.keys()
            ):
            ...
            create_md_response = requests.post(
                f"{BASE_URL}/tournaments/{t_alias}/seasons/{s_alias}/rounds/{r_alias}/matchdays/",
                json=jsonable_encoder(new_matchday),
                headers=headers,
            )
            if create_md_response.status_code != 201:
                print(
                    "Failed to create new matchday: ",
                    f"{t_alias} / {s_alias} / {r_alias} / {md_alias}",
                    " - Status code:",
                    create_md_response.status_code,
                )
                exit()
        ...
        home_team_response = requests.get(
            f"{BASE_URL}/clubs/{home_club_alias}/teams/{home_team_alias}", headers=headers
        )
        ...
        away_team_response = requests.get(
            f"{BASE_URL}/clubs/{away_club_alias}/teams/{away_team_alias}", headers=headers
        )
        ...
        if not match_exists:
            response = requests.post(f"{BASE_URL}/matches/", json=new_match_data, headers=headers)
            ...
            print(
                f"Match already exists in {t_alias} / {r_alias} / {md_alias}: {home.fullName} - {away.fullName}"
            )
