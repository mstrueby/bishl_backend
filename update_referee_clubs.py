...
for user in users:
    ...
    if user_club and (not user_referee or user_referee.get('club') != user_club):
        ...
        if update_result.modified_count > 0:
            updated_users += 1
            print(f"✅ Updated referee club for {user.get('firstName', '')} {user.get('lastName', '')} - {user_club.get('clubName', '')}")
        else:
            print(f"⚠️ No changes needed for {user.get('firstName', '')} {user.get('lastName', '')}")
    else:
        print(f"ℹ️ Skipping {user.get('firstName', '')} {user.get('lastName', '')} - No club data or already synced")
