...
parser.add_argument("--from_player_id", required=True, help="Player ID to merge from")
parser.add_argument("--to_player_id", required=True, help="Player ID to merge into")
...
async def merge_players():
    ...
    for from_stat in from_stats:
        ...
        for to_stat in merged_stats:
            if (
                to_stat["tournament"]["alias"] == from_stat["tournament"]["alias"]
                and to_stat["season"]["alias"] == from_stat["season"]["alias"]
                and to_stat["round"]["alias"] == from_stat["round"]["alias"]
                and to_stat["team"]["name"] == from_stat["team"]["name"]
            ):
                # Merge stats
