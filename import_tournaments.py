        ...
        for row in reader:
            ...
            if not tournament_exists:
                ...
                if response.status_code == 201:
                    ...
                    print(
                        "Failed to post Tournament: ", row, " - Status code:", response.status_code
                    )
                    exit()
            else:
    ...
    with open("data/data_seasons.csv", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            season_exists = db_collection.find_one(
                {"alias": row["t_alias"], "seasons.alias": row["alias"]}
            )
            if not season_exists:
                ...
                response = requests.post(
                    f"{BASE_URL}/tournaments/{row['t_alias']}/seasons/", json=row, headers=headers
                )
                if response.status_code == 201:
    ...
    with open("data/data_rounds.csv", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            round_exists = db_collection.find_one(
                {
                    ...
                    "seasons": {
                        "$elemMatch": {
                            "alias": row["s_alias"],
                            "rounds": {"$elemMatch": {"alias": row["alias"]}},
                        }
                    },
                }
            )
            if not round_exists:
                ...
                if response.status_code == 201:
                    print(
                        f"--> Successfully posted Round: {row['name']} / {row['t_alias']} / {row['s_alias']}"
                    )
                else:
    ...
    with open("data/data_matchdays.csv", encoding="utf-8") as f:
        ...
        for row in reader:
                    ...
                    "seasons": {
                        "$elemMatch": {
                            ...
                            "rounds": {
                                "$elemMatch": {
                                    "alias": row["r_alias"],
                                    "matchdays": {"$elemMatch": {"alias": row["alias"]}},
                                }
                            },
                        }
                    },
            ...
            if not matchday_exists:
                ...
                if response.status_code == 201:
                    print(
                        f"--> Successfully posted Matchday: {row['name']} / {row['t_alias']} / {row['s_alias']} / {row['r_alias']}"
                    )
                else:
...
parser.add_argument("--tournaments", action="store_true", help="Import tournaments.")
...
parser.add_argument("--matchdays", action="store_true", help="Import matchdays.")
