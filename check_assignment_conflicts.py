...
class AssignmentConflictChecker:
    ...
    async def check_assignment_conflicts(self) -> list[dict[str, Any]]:
        ...
        assignments = await self.db.assignments.find({"status": "ASSIGNED"}).to_list(None)
            ...
            if not match_referee:
                ...
                if assigned_entry:
                    ...
                    if issue_date:
                        ...
                        # Track oldest issue
                        if (
                            not analysis["oldest_issue_date"]
                            or issue_date < analysis["oldest_issue_date"]
                        ):
    ...
    async def print_ref_admin_analysis(self, analysis: dict[str, Any]):
        ...
        print(
            f"✅ Properly set in match: {properly_set} ({properly_set/total*100:.1f}%)"
            if total > 0
            else "✅ Properly set in match: 0"
        )
        print(
            f"❌ Missing from match: {missing} ({missing/total*100:.1f}%)"
            if total > 0
            else "❌ Missing from match: 0"
        )
        print(
            f"⚠️  Wrong referee in match: {wrong} ({wrong/total*100:.1f}%)"
            if total > 0
            else "⚠️  Wrong referee in match: 0"
        )

        if missing > 0 or wrong > 0:
            print(
                f"\n🚨 WORKFLOW SUCCESS RATE: {properly_set/total*100:.1f}%"
                if total > 0
                else "🚨 WORKFLOW SUCCESS RATE: N/A"
            )
            ...
            if analysis["issues_by_tournament"]:
                ...
                for tournament, count in sorted(
                    analysis["issues_by_tournament"].items(), key=lambda x: x[1], reverse=True
                ):
            ...
            if analysis["issues_by_referee"]:
                ...
                sorted_referees = sorted(
                    analysis["issues_by_referee"].items(),
                    key=lambda x: x[1]["total_issues"],
                    reverse=True,
                )
                for referee_name, referee_data in sorted_referees[:10]:
                    print(
                        f"  {referee_name} ({referee_data['club']}): {referee_data['total_issues']} issues"
                    )
                    ...
                    if referee_data["wrong_referee_in_match"] > 0:
                        print(
                            f"    - Wrong referee in match: {referee_data['wrong_referee_in_match']}"
                        )
            ...
            for issue in analysis["recent_issues"][:10]:
                print(
                    f"  {issue['assigned_date'].strftime('%Y-%m-%d %H:%M')} - {issue['referee_name']} (Pos {issue['position']}) for {issue['tournament']}"
                )
                print(f"    Assigned by: {issue['assigned_by']}")
                print(
                    f"    Match: {issue['match_date'].strftime('%Y-%m-%d %H:%M') if issue['match_date'] else 'No date'}"
                )
        else:
    ...
    async def print_conflicts_report(self, conflicts: list[dict[str, Any]]):
        ...
        print("\n" + "=" * 80)
        print("ASSIGNMENT-MATCH CONFLICTS REPORT")
        ...
        for conflict_type, type_conflicts in conflicts_by_type.items():
            ...
            for i, conflict in enumerate(type_conflicts, 1):
                ...
                print(
                    f"   Assigned Referee: {assigned_ref.get('firstName')} {assigned_ref.get('lastName')} ({assigned_ref.get('userId')})"
                )
                ...
                if conflict.get("match_referee"):
                    ...
                    print(
                        f"   Match Referee: {match_ref.get('firstName')} {match_ref.get('lastName')} ({match_ref.get('userId')})"
                    )
                ...
                if conflict.get("match_info"):
                    ...
                    print(
                        f"     Teams: {match_info.get('home_team')} vs {match_info.get('away_team')}"
                    )
                    if match_info.get("start_date"):
                        print(f"     Date: {match_info.get('start_date')}")
                    print("     Current Referees:")
                    if match_info.get("referee1"):
                        ref1 = match_info["referee1"]
                        print(
                            f"       Referee1: {ref1.get('firstName')} {ref1.get('lastName')} ({ref1.get('userId')})"
                        )
                    else:
                        print("       Referee1: Not set")
                    if match_info.get("referee2"):
                        ref2 = match_info["referee2"]
                        print(
                            f"       Referee2: {ref2.get('firstName')} {ref2.get('lastName')} ({ref2.get('userId')})"
                        )
                    else:
                        print("       Referee2: Not set")
...
async def main():
    ...
    try:
        print(
            f"Checking assignment conflicts in {'PRODUCTION' if args.prod else 'DEVELOPMENT'} database..."
        )
        ...
        if conflicts:
            ...
            print(
                "2. Update either the assignment record or the match document to resolve conflicts"
            )
        ...
        if workflow_issues > 0:
            ...
            if analysis["issues_by_referee"]:
                ...
                referees_with_multiple_issues = sum(
                    1
                    for ref_data in analysis["issues_by_referee"].values()
                    if ref_data["total_issues"] > 1
                )
                ...
                if referees_with_multiple_issues > 0:
                    print(
                        "   - This suggests the issue may be systemic rather than referee-specific"
                    )
                else:
                    print("   - Issues appear to be distributed across different referees")
