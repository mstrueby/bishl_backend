...
try:
    ...
    with open(filename, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # check player in db
            query = {"firstName": first_name, "lastName": last_name, "birthdate": birthdate}
            ...
            if exisiting_player:
                ...
                player_response = requests.post(
                    f"{BASE_URL}/players/", headers=headers, data=player.model_dump()
                )
            ...
            print(f"{first_name};{last_name};({birthdate});", ";".join(club_team_list))
            ...
            new_club_assignment = AssignedClubs(
                ...
                clubIshdId=(
                    club.ishdId if club.ishdId is not None else 0
            ...
            existing_club = next((x for x in assigned_clubs if x.clubId == str(club.id)), None)
