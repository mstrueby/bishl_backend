...
def import_rosters():
    ...
    roster_files = [file for file in roster_files if not file.endswith("_done.csv")]
            ...
            for match in matches:
                ...
                if existing_match:
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            "Failed to put Home Roster for Match ",
                            match_id,
                            " - Status code:",
                    ...
                    if response.status_code == 200:
                        ...
                        print(
                            "Failed to put Away Roster for Match ",
                            match_id,
                            " - Status code:",
                            response.status_code,
                        )
    ...
    with open("data/data_scores.csv", encoding="utf-8") as f:
        ...
        for i, row in enumerate(reader):
            ...
            if isinstance(row.get("assistPlayer"), str) and row["assistPlayer"]:
                ...
                assistPlayer = None
            current_score = {"matchTime": row["matchTime"], "goalPlayer": goalPlayer}
            if assistPlayer:
        ...
        for match in matches:
            ...
            response = requests.patch(
                f"{BASE_URL}/matches/{match_obj_id}", json=match, headers=headers
            )
            if response.status_code == 200:
...
parser.add_argument("--deleteAll", action="store_true", help="Delete all matches.")
...
parser.add_argument("--penalties", action="store_true", help="Import penalties.")
...
with open("data/data_matches.csv", encoding="utf-8") as f:
    ...
    for row in reader:
        ...
        if not match_exists:
            response = requests.post(f"{BASE_URL}/matches/", json=row, headers=headers)
            if response.status_code == 201:
                ...
                print("Failed to post Match: ", row, " - Status code:", response.status_code)
                exit()
        else:
            print(
