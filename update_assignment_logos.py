    ...
    if updates_needed:
        try:
            result = db["assignments"].update_one(
                {"_id": assignment["_id"]}, {"$set": updates_needed}
            )

            if result.modified_count > 0:
                updated_assignments += 1
                print(
                    f"✅ Updated logo for referee {referee.get('firstName', '')} {referee.get('lastName', '')}"
                )
            else:
                print(
                    f"⚠️ No changes made for referee {referee.get('firstName', '')} {referee.get('lastName', '')}"
                )
        except Exception as e:
            print(
                f"❌ Error updating {referee.get('firstName', '')} {referee.get('lastName', '')}: {str(e)}"
            )
    else:
        print(
            f"ℹ️ No updates needed for referee {referee.get('firstName', '')} {referee.get('lastName', '')}"
        )
